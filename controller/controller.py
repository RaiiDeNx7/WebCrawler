...
def status():
    with queue_lock:
        return jsonify({
            ...
            'queue_contents': all_urls_in_queue  # Show tracked URLs
        })
            ...
            if msg_type == MSG_READY:
                with queue_lock:
                    if not url_queue.empty():
                        ...
                        conn.sendall(encode_message(MSG_URL, next_url))
                    else:
                ...
                with queue_lock:
                    for link in links:
                        if link not in visited_urls:
                            url_queue.put(link)
                            all_urls_in_queue.append(link)  # Track the URL in the all_urls_in_queue list
...
def main():
    # Start worker server in a background thread
